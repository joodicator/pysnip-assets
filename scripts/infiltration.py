...
ATTACKER_TEAM = 0 # 0 = blue, 1 = green
...
def apply_script(protocol, connection, config):
    class InfiltrationConnection(connection):
        def on_team_join(self, team):
            ...
            defender_size = defender_count * ATTACKER_TO_DEFENDER_RATIO

            if attacker.count() != 0 != defender.count() \
            and (attacker_count > defender_size and team is attacker \
              or attacker_count < defender_size and team is defender):
                self.send_chat(S_TEAM_FULL)
                self.protocol.attacker_count = \
                    attacker.count() + (1 if team is defender else 0)
                self.protocol.defender_count = \
                    defender.count() + (1 if team is attacker else 0)
                self.protocol.update_defender_score_loop()
                return False
            else:
                self.protocol.attacker_count = attacker_count
                self.protocol.defender_count = defender_count
                self.protocol.update_defender_score_loop()
        ...
        def on_team_changed(self, old_team):
            ...
            self.protocol.attacker_count = self.protocol.attacker.count()
            self.protocol.defender_count = self.protocol.defender.count()
            self.protocol.update_defender_score_loop()
        ...
        def on_flag_take(self):
            ...
            self.protocol.flag_held = True
            self.protocol.update_defender_score_loop()
            return connection.on_flag_take(self)
        
        def on_flag_drop(self):
            if self.team is self.protocol.attacker:
                self.protocol.flag_held = False
                self.protocol.update_defender_score_loop()
    ...
    class InfiltrationProtocol(protocol):
        ...
        def on_map_change(self, map):
            ...
            self.attacker_count = self.attacker.count()
            self.defender_count = self.defender.count()
            self.flag_held = False
            protocol.on_map_change(self, map)
            self.update_defender_score_loop()
        
        def on_map_leave(self):
            self.stop_defender_score_loop()
        ...
        def stop_defender_score_loop(self):
        ...
        def update_defender_score_loop(self):
            if self.defender_score_loop and self.defender_score_loop.running:
                if self.attacker_count == 0 or self.defender_count == 0 \
                or  self.flag_held:
                    self.stop_defender_score_loop()
            else:
                if self.attacker_count > 0 and self.defender_count > 0 \
                and not self.flag_held:
